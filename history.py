...
history=pd.read_csv(path+"mega_data.csv",sep=',',encoding="utf-8",error_bad_lines=False,low_memory = False)
print(mega_data.shape)

history,abc_data=train_test_split(history, test_size = 0.6, random_state =9, stratify=history['damage_grade'])
...
district_list=history.district_id.unique().tolist()

### COUNT
#Creation has_features_count Columns
for i in has_features:
    history[i+'_district_count']=0
for i in has_features:
    for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        ...
        history[i+'_district_count']=np.where(history['district_id']==dist, value, history[i+'_district_count'])

### RATIO 
#### [No of Records Having 1] / [Total Number of Records]
#Creation has_features_ratio Columns
for i in has_features:
    history[i+'_district_ratio']=0        
for i in has_features:
    for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        ...
        history[i+'_district_ratio']=np.where(history['district_id']==dist, value, history[i+'_district_ratio'])

### AVERAGE RATIO
#### { [Number of Records Having 0] / [Total Number of Records] } / { [Number of Records Having 1] / [Total Number of Records] }
#Creation has_features_ratio Columns
for i in has_features:
    history[i+'_district_average_ratio']=0
for i in has_features:
    for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        ...
        history[i+'_district_average_ratio']=np.where(history['district_id']==dist, value, history[i+'_district_average_ratio'])
...
has_features=['has_geotechnical_risk_fault_crack','has_geotechnical_risk_flood','has_geotechnical_risk_land_settlement',
       ...
       'has_superstructure_bamboo', 'has_superstructure_rc_non_engineered','has_superstructure_rc_engineered',
        'has_superstructure_other']
municipal_list=history.vdcmun_id.unique().tolist()

### COUNT
#Creation has_features_count Columns
for i in has_features:
    history[i+'_municipal_count']=0
for i in has_features:
    for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        temp_new.drop('index', axis=1, inplace=True)
        ...
        history[i+'_municipal_count']=np.where(history['vdcmun_id']==mun, value, history[i+'_municipal_count'])

### RATIO 
#### [No of Records Having 1] / [Total Number of Records]
#Creation has_features_ratio Columns
for i in has_features:
    history[i+'_municipal_ratio']=0
for i in has_features:
    for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        ...
        history[i+'_municipal_ratio']=np.where(history['vdcmun_id']==mun, value, history[i+'_municipal_ratio'])

### AVERAGE RATIO
#### { [Number of Records Having 0] / [Total Number of Records] } / { [Number of Records Having 1] / [Total Number of Records] }
#Creation has_features_ratio Columns
for i in has_features:
    history[i+'_municipal_average_ratio']=0
for i in has_features:
    for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        ...
        history[i+'_municipal_average_ratio']=np.where(history['vdcmun_id']==mun, value, history[i+'_municipal_average_ratio'])
### AVERAGE NUMBER OF FLOORS BEFORE EARTHQUAKE  DISTRICT, MUNICIPAL and WARD
#List On District, Municipal and Ward Level
district_list=history.district_id.unique().tolist()
municipal_list=history.vdcmun_id.unique().tolist()
ward_list=history.ward_id_x.unique().tolist()
#District LEvel
history['avg_floors_before_quake_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_floors_before_quake_district']=np.where(history['district_id']==dist, value, history['avg_floors_before_quake_district'])
#Municipal LEvel
history['avg_floors_before_quake_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_floors_before_quake_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_floors_before_quake_municipal'])
#ward level
history['avg_floors_before_quake_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        temp_new.drop('index' ,  axis=1 , inplace = True)
        
        number_of_floors=temp_new['count_floors_pre_eq'].sum()
        value = number_of_floors/len(temp_new)
        history['avg_floors_before_quake_ward']=np.where(history['ward_id_x']==ward, value, history['avg_floors_before_quake_ward'])


### AVERAGE NUMBER OF FLOORS AFTER EARTHQUAKE  DISTRICT, MUNICIPAL and WARD
#District LEvel
history['avg_floors_after_quake_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_floors_after_quake_district']=np.where(history['district_id']==dist, value, history['avg_floors_after_quake_district'])
#Municipal LEvel
history['avg_floors_after_quake_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_floors_after_quake_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_floors_after_quake_municipal'])

#Ward LEvel
history['avg_floors_after_quake_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_floors_after_quake_ward']=np.where(history['ward_id_x']==ward, value, history['avg_floors_after_quake_ward'])

### AVERAGE NUMBER OF FAMILIES IN A DISTRICT & MUNICIPAL
#district level 
history['avg_number_families_in_district']=0
history['number_of_families_in_district']=0
for dist in district_list:
    temp_new=pd.DataFrame()
    temp_new=history.loc[history.district_id== dist,:]
    temp_new=temp_new.reset_index()
    ...
    history['avg_number_families_in_district']=np.where(history['district_id']==dist, value, history['avg_number_families_in_district'])
    history['number_of_families_in_district']=np.where(history['district_id']==dist, number_of_families, history['number_of_families_in_district'])    

# muncipal level
    
history['avg_number_families_in_municipal']=0
history['number_of_families_in_municipal']=0
for mun in municipal_list:
    temp_new=pd.DataFrame()
    temp_new=history.loc[history.vdcmun_id== mun,:]
    temp_new=temp_new.reset_index()
    ...
    history['avg_number_families_in_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_number_families_in_municipal'])
    history['number_of_families_in_municipal']=np.where(history['vdcmun_id']==mun, number_of_families, history['number_of_families_in_municipal'])    

#ward level 
    
history['avg_number_families_in_ward']=0
history['number_of_families_in_ward']=0
for ward in ward_list:
    temp_new=pd.DataFrame()
    temp_new=history.loc[history.ward_id_x== ward,:]
    temp_new=temp_new.reset_index()
    ...
    number_of_families=temp_new['count_families'].sum()
    value=number_of_families/len(temp_new)
    history['avg_number_families_in_municipal']=np.where(history['ward_id_x']==ward, value, history['avg_number_families_in_municipal'])
    history['number_of_families_in_municipal']=np.where(history['ward_id_x']==ward, number_of_families, history['number_of_families_in_municipal'])    

### AVERAGE Height OF Building BEFORE EARTHQUAKE  DISTRICT, MUNICIPAL and WARD level
#District LEvel
history['avg_height_of_building_before_quake_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_height_of_building_before_quake_district']=np.where(history['district_id']==dist, value, history['avg_height_of_building_before_quake_district'])
#Municipal LEvel
history['avg_height_of_building_before_quake_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_height_of_building_before_quake_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_height_of_building_before_quake_municipal'])
#ward level
history['avg_height_of_building_before_quake_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        ...
        value = number_of_floors/len(temp_new)
        history['avg_height_of_building_before_quake_ward']=np.where(history['ward_id_x']==ward, value, history['avg_height_of_building_before_quake_ward'])


### AVERAGE Height OF Building AFTER EARTHQUAKE  DISTRICT, MUNICIPAL and WARD level

#District LEvel
history['avg_height_of_building_after_quake_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_height_of_building_after_quake_district']=np.where(history['district_id']==dist, value, history['avg_height_of_building_after_quake_district'])
#Municipal LEvel
history['avg_height_of_building_after_quake_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_height_of_building_after_quake_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_height_of_building_after_quake_municipal'])

#Ward LEvel
history['avg_height_of_building_after_quake_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_height_of_building_after_quake_ward']=np.where(history['ward_id_x']==ward, value, history['avg_height_of_building_after_quake_ward'])

#Average age of building in district , muncipal and ward level 
  
#District LEvel
history['avg_age_of_building_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_age_of_building_district']=np.where(history['district_id']==dist, value, history['avg_age_of_building_district'])
#Municipal LEvel
history['avg_age_of_building_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_age_of_building_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_age_of_building_municipal'])

#Ward LEvel
history['avg_age_of_building_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_age_of_building_ward']=np.where(history['ward_id_x']==ward, value, history['avg_age_of_building_ward'])      

#Average plinth area of building in district , muncipal and ward level 

  
#District LEvel
history['avg_plinth_area_of_building_district']=0
for dist in district_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.district_id == dist, :]
        temp_new = temp_new.reset_index()
       ...
       plinth_area__of_building=temp_new['plinth_area_sq_ft'].sum()
       value=plinth_area__of_building/len(temp_new)
        
        history['avg_plinth_area_of_building_district']=np.where(history['district_id']==dist, value, history['avg_plinth_area_of_building_district'])
#Municipal LEvel
history['avg_plinth_area_of_building_municipal']=0
for mun in municipal_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.vdcmun_id == mun, :]
        temp_new = temp_new.reset_index()
       ...
       value=plinth_area__of_building/len(temp_new)
        ...
        history['avg_plinth_area_of_building_municipal']=np.where(history['vdcmun_id']==mun, value, history['avg_plinth_area_of_building_municipal'])

#Ward LEveL
history['avg_plinth_area_building_ward']=0
for ward in ward_list:
        temp_new = pd.DataFrame()
        temp_new = history.loc[history.ward_id_x == ward, :]
        temp_new = temp_new.reset_index()
        ...
        history['avg_plinth_area_building_ward']=np.where(history['ward_id_x']==ward, value, history['avg_plinth_area_building_ward'])      
...
>>>>>>> 522e0b68f0b72d34c95b39c9ed33e47c741ccbc9
